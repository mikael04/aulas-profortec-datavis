...
::: incremental
-   Apresentação dos tutores e alunos
...
# We need to recalculate the 'media' column for the plot, as it was
# removed during the table-making process above.
df <- df |>
  dplyr::group_by(idade) |>
  dplyr::mutate(media = mean(n_filhos)) |>
  dplyr::ungroup()
...
p <- ggplot(df) +
  geom_violin(aes(x = idade, y = n_filhos), linetype = 1, linewidth = 0.5) +
  geom_hline(
    aes(yintercept = mean(media), color = "Média"),
    linetype = 2,
    linewidth = 0.5
  ) +
  ...
  theme_minimal() +
  labs(
    title = "Distribuição de número de filhos por categoria de idade",
    x = "Idade das mães",
    y = "Número de filhos",
    color = NULL
  )
